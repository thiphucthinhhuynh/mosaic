...
For the narrative reasoning (context, alternatives, trade-offs) behind the most significant decisions, see [docs/adr/](adr/README.md). This document is the current-state reference; the ADRs are the historical record of *why*.
...
| Backend framework | Express + TypeScript | Layered internally, see §5 |
...
- **Forms** use React Hook Form with a Zod resolver, using the *same* Zod schema the backend validates against (imported from `packages/shared`).
...
- Every async route handler is wrapped by a shared `asyncHandler` utility so a rejected promise always reaches the centralized handler (a well-known Express footgun otherwise).
