...
- Endpoint contracts are documented per domain under `docs/api/` as they're implemented; see [docs/api/authentication.md](api/authentication.md) for the first one, and [docs/api/items.md](api/items.md) for the nested-resource case (`/stores/:storeId/items` to list and create, `/items/:id` to read, update, and delete).
...
- **Ownership through a relation (Milestone 4) needed zero changes to `requireOwnership`.** `Item` has no `ownerId`; `PUT`/`DELETE /items/:id` pass the middleware `findItemOwnerId` (`items.repository.ts`), which selects `item.store.ownerId` and flattens it to `{ ownerId }`, and `POST /stores/:storeId/items` reuses the store loader. The middleware reads `req.params.id` on both routers, which is why the store's item routes name their param `:id` internally even though the URL and docs call it `:storeId`. An integration test proves a second user cannot update or delete another user's item.
- **Money is a string end to end.** `price` is `Decimal(10, 2)` in Postgres, a `Prisma.Decimal` in the repository layer, a JSON string on the wire (`"24.99"`, via `Decimal.toJSON()`), and typed `string` in `packages/shared`'s `PublicItem`. The frontend's `formatPrice` parses it for display only. `createItemSchema` caps `price` at the column's maximum so an over-cap value is a clean `400` rather than a database error.
- **Multi-row writes are transactional.** Creating an item with images, and replacing an item's images on update, each run in one `prisma.$transaction`. `updateItemSchema`'s `imageUrls` is a _replacement set_ (present → the item's images become exactly that list; omitted → untouched) rather than a patch, because the edit form submits the full list and no add/remove-by-id operation is needed.
- **Vitest runs test files serially (`fileParallelism: false`).** Test files share one real Postgres database, and a concurrent insert from another file could shift a paginated query's window mid-test — a real flake surfaced while adding the item tests. Serial files trade a few seconds of suite time for determinism; the reasoning is documented next to the setting in `apps/api/vitest.config.ts`.
- **Frontend item UI shares one field set and picks its validation schema by what the form submits.** `ItemFormFields` holds the fields (including the repeatable image-URL list, driven by `watch`/`setValue` because `useFieldArray` can't hold plain strings) for both create and edit. The edit form validates with `createItemSchema`, not `updateItemSchema`, since it is pre-filled with and always submits every field. Numeric inputs use `valueAsNumber` so the schema sees real numbers, and `price`/`quantity` carry readable Zod messages because an empty number input otherwise surfaces Zod's developer-facing "expected number, received NaN".
- **Item cache handling avoids a stale flash and a false 404.** After an edit, the PUT response (a `PublicItem` with no embedded store) is merged into the cached `['items', 'detail', id]` entry so the detail page shows new values immediately; the store's `['items', 'list', storeId]` pages are invalidated. After a delete, the list is invalidated but the deleted item's detail query is deliberately left alone — the detail page is still mounted when the mutation resolves, and invalidating it would refetch and briefly show "Item not found" before navigation lands.
