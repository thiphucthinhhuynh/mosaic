...
- [x] **7. Frontend — Edit/Delete Item UI:** edit form (pre-filled, including existing image URLs), delete confirmation, both shown only to the item's owner (i.e. the store's owner). — Done: new `/items/:id/edit` route (`<ProtectedRoute>`-gated; `EditItemPage` redirects a non-owner back to the item, with the API's `requireOwnership` still the real boundary — same split as `EditStorePage`/`CreateItemPage`). An item's owner is `item.store.owner.id`, so the "Edit item" link and `DeleteItemButton` on `ItemDetailPage` render only when that matches the signed-in user. The create form's field set was extracted into a shared `ItemFormFields` component (the repeatable image-URL list is too fiddly to maintain twice); `EditItemForm` validates with `createItemSchema`, not `updateItemSchema`, because it is pre-filled with every field and always submits all of them, and it sends the full `imageUrls` list, which the API treats as a replacement set. `useUpdateItemMutation` merges the PUT response (a `PublicItem`, no embedded store) into the cached detail so the detail page shows the new values immediately, and invalidates the store's `['items', 'list', storeId]` pages. `useDeleteItemMutation` invalidates that list but deliberately leaves the item's detail query alone — the detail page is still mounted when it runs, so invalidating/removing it would refetch and flash "Item not found" before navigation lands. Typecheck, lint, format, and production build pass, and the whole flow — including `CreateItemForm`'s refactor onto `ItemFormFields` — was exercised in a real browser in step 8's walkthrough.
- [x] **8. Full milestone verification & docs:** complete gate (lint/typecheck/test/build/format), live Playwright walkthrough (browse a store's items as guest → sign in as the store owner → create an item with images → edit it → delete it; separately, confirm a non-owner cannot edit/delete via the UI), then update this roadmap entry, `docs/architecture.md`, a new `docs/api/items.md`, and `README.md`. — Done: **Gate:** typecheck, lint, format check, and production build all green; **80/80** backend integration tests pass (this was the first full run of the step 4 update/delete tests, including the explicit "user B cannot edit or delete user A's item" cases). **Live walkthrough** in real Chromium (driven with `playwright-core`, installed outside the repo — no dependency was added) against the live API and Vite dev server on the dev database, 26/26 checks: a guest browses a seeded store's items and an item's detail page and sees no Edit/Delete controls; a signed-up owner creates a store (empty-items state shown), then an item with three image URLs — an empty submit shows field errors and stays put, removing the middle image removes the right row — landing on the detail page with formatted price, stock, and both remaining images; editing opens a form pre-filled with every field and both image URLs, rejects a negative price client-side, and on save returns to the detail page with the new name/price/stock and the _replaced_ image set, persisting across a reload and showing in the store's list; a second signed-in user sees no Edit/Delete controls, is redirected when opening `/items/:id/edit` directly, and gets `403` on both `PUT` and `DELETE /items/:id` sent with their session, while a signed-out visitor at `/edit` is sent to `/login`; the owner cancels a delete (item kept), confirms one (returns to the store page, item gone from the list), and visiting the deleted item's URL shows "Item not found." **Limits of that walkthrough:** the "no false 404 flash after delete" check asserts the final state, not the absence of a transient flash, so that design choice (see `docs/architecture.md` §19) rests on the reasoning rather than a frame-level check; and the API-error path inside the forms (`formError` surfacing a 403/500) is still covered only by the HTTP-level backend tests. The walkthrough's throwaway users (and their store/item, via cascade) were deleted afterwards; the dev database is back to its seeded state. **Docs:** new `docs/api/items.md`; `docs/api/stores.md` cross-references the nested item routes and no longer describes the ownership-through-a-relation case as future work; `docs/architecture.md` marks `Item`/`ItemImage` as migrated (they were missing their ✅ since step 1) and gains §19 notes on ownership through a relation, money-as-string, transactional multi-row writes, serial test files, the shared item form, and the item cache handling; `README.md` links the new API doc and its status line now includes Milestone 4.
