...
- [ ] **2. Backend — public reads (Guest tier):** `GET /api/v1/stores/:storeId/items` (paginated, following the stores-list pattern), `GET /api/v1/items/:id`. New `items` module (repository/service/controller/routes/barrel), embedding images and enough store/owner info for the frontend's item detail page. Integration tests: happy path, pagination, 404 (both a bad item id and a bad `:storeId`), invalid id.
