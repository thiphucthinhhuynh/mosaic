...
- Prisma's `include` conveniences can hide N+1 query patterns if used carelessly; the project deliberately writes at least one raw-SQL query somewhere in the codebase so the team (i.e., the author) stays fluent in what the ORM is abstracting away, and audits for N+1s explicitly in Milestone 15.
...
- [docs/roadmap.md](../roadmap.md) — Milestone 1 (first schema), Milestone 15 (N+1 audit)
